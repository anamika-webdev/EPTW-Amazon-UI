...
const SignupPage: React.FC<SignupPageProps> = ({ onLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate all fields
    const isLoginIdValid = validateField('login_id', formData.login_id);
    const isFullNameValid = validateField('full_name', formData.full_name);
    const isEmailValid = validateField('email', formData.email);
    const isPasswordValid = validateField('password', formData.password);
    const isConfirmPasswordValid = validateField('confirmPassword', formData.confirmPassword);

    if (!isLoginIdValid || !isFullNameValid || !isEmailValid || !isPasswordValid || !isConfirmPasswordValid) {
    ...
    try {
      ...
      if (err.response?.status === 409) {
        setError(errorMessage || 'User already exists with this login ID or email');
      ...
      <div className="w-full max-w-md p-10 space-y-8 bg-white shadow-xl rounded-2xl">
        ...
        <form className="mt-8 space-y-4" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            />
            {validationErrors.login_id && (
              <p className="mt-1 text-xs text-red-600">{validationErrors.login_id}</p>
            )}
          </div>
          ...
          <div>
            ...
            <select
              ...
              value={formData.role}
              onChange={handleChange}
              className="relative block w-full px-4 py-3 text-gray-900 border border-gray-300 rounded-lg appearance-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 focus:z-10 sm:text-sm"
            >
