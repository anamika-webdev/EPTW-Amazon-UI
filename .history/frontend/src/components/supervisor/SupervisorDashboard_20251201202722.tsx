// frontend/src/components/supervisor/SupervisorDashboard.tsx - UPDATED VERSION
// Removed "Initiated PTWs" - All submitted forms appear in "Approved PTWs"

import { useState, useEffect } from 'react';
import { Button } from '../ui/button';
import { FileText, Users, CheckCircle2, Clock, XCircle, Play, AlertCircle } from 'lucide-react';
...
export function SupervisorDashboard({ onNavigate }: SupervisorDashboardProps) {
  ...
  // Approved PTWs = All submitted permits (Pending_Approval or Active)
  ...
  const PermitTable = ({ permits, title, emptyMessage }: { permits: Permit[]; title: string; emptyMessage: string }) => (
            ...
            <tbody className="divide-y divide-slate-200">
              {permits.map((permit) => (
                <tr key={permit.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3">
                    <Button
                      onClick={() => console.log('View permit:', permit.id)}
                      variant="outline"
                      size="sm"
                    >
                      View
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Stats Grid - 5 cards instead of 6 */}
      ...
      {/* Approved PTWs - Shows all submitted permits (Pending_Approval + Active) */}
      <PermitTable
