...
// Helper function to execute queries (works with both pool and direct connection)
const executeQuery = async (query, params) => {
  // If db is a pool, use getConnection
  ...
  // Otherwise, use db.query directly (promise-based)
  else if (typeof db.query === 'function') {
    return await db.query(query, params);
  }
  // Otherwise, use db.execute (promise-based)
  else if (typeof db.execute === 'function') {
    return await db.execute(query, params);
  }
  else {
    throw new Error('Database connection method not found');
  }
};
