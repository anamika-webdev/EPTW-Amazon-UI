...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const [lastPermit] = await connection.query(
      'SELECT permit_number FROM permits ORDER BY id DESC LIMIT 1'
    );
    
    let newSerialNumber = 'PTW-0001';
    if (lastPermit.length > 0 && lastPermit[0].permit_number) {
      const lastNumber = parseInt(lastPermit[0].permit_number.split('-').pop());
